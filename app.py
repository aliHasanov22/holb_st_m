...
def sync_holberton_projects(session_cookie):
    ...
    for project_div in soup.select('.list-group-item'):
        name_tag = project_div.select_one('h4')
        deadline_tag = project_div.select_one('.deadline')
        if not name_tag:
            continue
        name = name_tag.text.strip()
        deadline_raw = deadline_tag.text.strip() if deadline_tag else ''
        deadline_date = parse_date_from_text(deadline_raw)
        projects.append({'title': name, 'deadline': deadline_date})
...
def sync_holberton_stats(session_cookie):
    ...
    for label in soup.select('.list-group-item, .panel, .card'):
        text = label.get_text(' ', strip=True).lower()
        if 'attendance' in text and attendance is None:
            attendance_match = re.search(r'(\d+(\.\d+)?)\s*%', text)
            if attendance_match:
                attendance = float(attendance_match.group(1))
        if 'average' in text and average is None:
            average_match = re.search(r'(\d+(\.\d+)?)\s*%', text)
            if average_match:
                average = float(average_match.group(1))

    return {'attendance': attendance, 'average': average}
...
def sync_intranet():
    ...
    if projects is None:
        return jsonify({'error': 'Unable to fetch projects.'}), 502
